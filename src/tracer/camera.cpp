...
namespace tracer {
  namespace camera {
    ...
    projective::projective(
      ...
      : camera(cam_to_world), tf_cam_to_ndc(cam_to_ndc), img_res(img_res)
    {
      tf_ndc_to_raster = tf::ndc_to_raster(img_res);
      tf_raster_to_ndc = tf_ndc_to_raster.inverse();
      tf_raster_to_cam = tf_cam_to_ndc.inverse() * tf_raster_to_ndc;
    }

    ortho::ortho(const matrix4f& cam_to_world, const vector2i& img_res, Float near, Float far)
      : projective(cam_to_world, tf::ortho(near, far), img_res),
      near(near), far(far) {}
    ...
    persp::persp(
      ...
      : projective(cam_to_world, tf::persp(near, far, fovy), img_res),
